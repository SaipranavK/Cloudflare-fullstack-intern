...
async function handleRequest(request) {
	...
	if(cookie == '0'){
		...
		const out = await fetch(api_urls_arr[1]).
		then((response) => {
		...
		//res.headers.set('Set-Cookie', 'prev_url=1');

		rewrite = new HTMLRewriter().
		on('title', new ElementHandler1()).on('p#description', new ElementHandler1()).on('h1#title', new ElementHandler1()).on('a#url', new ElementHandler1()).transform(res);
		...
		const out = await fetch(api_urls_arr[0]).
		then((response) => {
			return response.text();
		})
		...
		const res = new Response(out);
		res.headers.set('Content-Type', 'text/html');
		//res.headers.set('Set-Cookie', 'prev_url=0');

		rewrite = new HTMLRewriter().
		on('title', new ElementHandler2()).on('p#description', new ElementHandler2()).on('h1#title', new ElementHandler2()).on('a#url', new ElementHandler2()).transform(res);
		...
		const out = await fetch(api_urls_arr[route]).
		then((response) => {
			return response.text();
		})
